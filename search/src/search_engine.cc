...
string SearchEngine::ros_plan_if_necessary() const {
    if (found_solution()){
        return ros_plan(get_plan());
    }else{
	return string("no plan");
    }
}

string SearchEngine::ros_plan_cost_if_necessary() const {
    if (found_solution()){
        return ros_plan_cost(get_plan(), 0);
    }else{
	return string("no plan");
    }
}
